...
from ra_ov3dseg.models.cylindrical_voxelization import (
    CylindricalVoxelizationConfig,
    make_cylinder_point_features,
    voxelize_cylinder_point_features,
)
from ra_ov3dseg.models.voxelization import hash_spconv_indices
...
def _import_spconv():
    try:
        ...
        raise ImportError(
            "cylinder_spconv_unet requires spconv. Install a CUDA-matched wheel, "
            "for example `pip install spconv-cu120` for a CUDA 12.x environment."
        ) from exc
    return spconv


class CylinderAsymResidualBlock(nn.Module):
    """Cylinder3D-style asymmetric residual block over [z, phi, radius] voxels."""
...
class CylinderSpConvUNet(nn.Module):
    """Cylinder-grid sparse U-Net baseline for outdoor LiDAR segmentation.

    This is the stronger default baseline path for V15. It keeps the project in
    one repo/env, but uses the core idea from mature outdoor LiDAR segmentors:
    cylindrical partitioning plus sparse 3D convolution and LiDAR intensity.
    """
    ...
    def __init__(
        ...
        self.spconv = spconv
        self.voxel_config = CylindricalVoxelizationConfig(voxel_size=voxel_size, point_cloud_range=point_cloud_range)
        ...
        self.stem = sparse_conv_norm_relu(spconv, input_channels, c0, indice_key="cyl_stem")
        self.enc0 = nn.ModuleList(
            [CylinderAsymResidualBlock(spconv, c0, "cyl_enc0_a"), CylinderAsymResidualBlock(spconv, c0, "cyl_enc0_b")]
        )
        self.down1 = sparse_downsample(spconv, c0, c1, indice_key="cyl_down1")
        self.enc1 = nn.ModuleList(
            [CylinderAsymResidualBlock(spconv, c1, "cyl_enc1_a"), CylinderAsymResidualBlock(spconv, c1, "cyl_enc1_b")]
        )
        self.down2 = sparse_downsample(spconv, c1, c2, indice_key="cyl_down2")
        self.enc2 = nn.ModuleList(
            [CylinderAsymResidualBlock(spconv, c2, "cyl_enc2_a"), CylinderAsymResidualBlock(spconv, c2, "cyl_enc2_b")]
        )
        self.down3 = sparse_downsample(spconv, c2, c3, indice_key="cyl_down3")
        self.bottleneck = nn.ModuleList(
            [
                CylinderAsymResidualBlock(spconv, c3, "cyl_bottleneck_a"),
                CylinderAsymResidualBlock(spconv, c3, "cyl_bottleneck_b"),
            ]
        )

        self.up3 = sparse_inverse_upsample(spconv, c3, c2, indice_key="cyl_down3")
        self.fuse2 = sparse_conv_norm_relu(spconv, c2 * 2, c2, indice_key="cyl_fuse2")
        self.fallback2 = sparse_conv_norm_relu(spconv, c2, c2, indice_key="cyl_fallback2")
        self.dec2 = CylinderAsymResidualBlock(spconv, c2, "cyl_dec2")

        self.up2 = sparse_inverse_upsample(spconv, c2, c1, indice_key="cyl_down2")
        self.fuse1 = sparse_conv_norm_relu(spconv, c1 * 2, c1, indice_key="cyl_fuse1")
        self.fallback1 = sparse_conv_norm_relu(spconv, c1, c1, indice_key="cyl_fallback1")
        self.dec1 = CylinderAsymResidualBlock(spconv, c1, "cyl_dec1")

        self.up1 = sparse_inverse_upsample(spconv, c1, c0, indice_key="cyl_down1")
        self.fuse0 = sparse_conv_norm_relu(spconv, c0 * 2, c0, indice_key="cyl_fuse0")
        self.fallback0 = sparse_conv_norm_relu(spconv, c0, c0, indice_key="cyl_fallback0")
        self.dec0 = CylinderAsymResidualBlock(spconv, c0, "cyl_dec0")
    ...
    def _gather_to_points(
        ...
        final_hash = hash_spconv_indices(sparse_tensor.indices, self.voxel_config.spatial_shape_zpr)
        input_hash = hash_spconv_indices(input_voxel_coords, self.voxel_config.spatial_shape_zpr)
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        point_features_in = make_cylinder_point_features(point_xyz, point_input_features, self.voxel_config)
        voxelized = voxelize_cylinder_point_features(
            point_xyz=point_xyz,
