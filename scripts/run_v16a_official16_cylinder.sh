...
EXPERIMENT_NAME="trainval_v16a_official16_cylinder_128"
TRAIN_START_IDX=0
...
usage() {
  cat <<'EOF'
Run V16a official-16 nuScenes lidarseg cylinder baseline.

Task type:
  - Precheck/class-frequency: CPU, a few minutes.
  - Training/prediction: GPU if --device cuda, usually H100.
  - Evaluation: CPU-light plus file IO.

Expected runtime:
  - Smoke: --train_max_samples 8 --eval_max_samples 8 --epochs 2 -> 5-15 min on one H100.
  - Default: 128 train / 128 eval / 10 epochs -> about 20-60 min on one H100.
  - 1024 train / 512 eval / 30 epochs -> several hours on one H100.

Main purpose:
  Fix the two V15 issues before mature-backbone vendoring:
    1) train/evaluate official nuScenes lidarseg 16-class space
    2) expand cylinder range to avoid dropping ~5% points

Examples:
  bash scripts/run_v16a_official16_cylinder.sh

  bash scripts/run_v16a_official16_cylinder.sh \
    --experiment_name trainval_v16a_official16_cylinder_smoke \
    --train_max_samples 8 \
    --eval_max_samples 8 \
    --epochs 2 \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cylinder_r_max) CYLINDER_R_MAX="$2"; shift 2 ;;
    --cylinder_z_min) CYLINDER_Z_MIN="$2"; shift 2 ;;
    --cylinder_z_max) CYLINDER_Z_MAX="$2"; shift 2 ;;
    --no_amp) AMP=0; shift ;;
    --no_augment) AUGMENT=0; shift ;;
    --skip_existing) SKIP_EXISTING=1; shift ;;
    --skip_precheck) SKIP_PRECHECK=1; shift ;;
...
mkdir -p "${TRAINING_DIR}" "${PREDICTION_DIR}" "${EVALUATION_DIR}" "${REPORT_DIR}" "${LOG_DIR}"

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="${LOG_DIR}/v16a_${EXPERIMENT_NAME}_${TIMESTAMP}.log"
LATEST_LOG="${LOG_DIR}/v16a_${EXPERIMENT_NAME}_latest.log"
...
PRECHECK_COMMAND=(
  ...
  --cylinder_point_cloud_range 0.0 -3.141592653589793 "${CYLINDER_Z_MIN}" "${CYLINDER_R_MAX}" 3.141592653589793 "${CYLINDER_Z_MAX}"
...
TRAIN_CORE=(
  ...
  --class_names_path "${CLASS_NAMES_PATH}"
  --split_config "${SPLIT_CONFIG}"
  --backbone cylinder_spconv_unet
  --device "${DEVICE}"
  --epochs "${EPOCHS}"
  --batch_size "${BATCH_SIZE}"
  --num_workers "${NUM_WORKERS}"
  --max_points "${MAX_POINTS}"
  --voxel_size 0.125 0.017453292519943295 0.25
  --point_cloud_range 0.0 -3.141592653589793 "${CYLINDER_Z_MIN}" "${CYLINDER_R_MAX}" 3.141592653589793 "${CYLINDER_Z_MAX}"
...
CHECKPOINT="${TRAINING_DIR}/cylinder_spconv_unet_best.pt"
PREDICT_COMMAND=(
...
SUMMARY_COMMAND=(
  ...
  --stage v16a_official16_cylinder
  --experiment_dir "${EXPERIMENT_DIR}"
  --output_json "${EXPERIMENT_DIR}/compact_summary.json"
)

{
  echo "[INFO] V16a official-16 cylinder baseline started at $(date -Is)"
  echo "[INFO] task_type=CPU precheck/class-frequency + GPU train/predict + eval"
  echo "[INFO] expected_runtime=default 128 train/eval, 10 epochs: about 20-60 min on one H100"
  echo "[INFO] gpu_required=yes for training when --device cuda; precheck itself is CPU-only"
  ...
  echo "[INFO] cylinder_range=r:[0,${CYLINDER_R_MAX}] phi:[-pi,pi] z:[${CYLINDER_Z_MIN},${CYLINDER_Z_MAX}]"
...
if [[ "${SKIP_PRECHECK}" != "1" ]]; then
  run_step "pre_v16a_sanity_check" "${PRECHECK_COMMAND[@]}"
fi
...
if [[ "${SKIP_TRAIN}" != "1" ]]; then
  run_step "train_v16a_official16_cylinder" "${TRAIN_COMMAND[@]}"
fi
if [[ "${SKIP_PREDICT}" != "1" ]]; then
  ...
  run_step "predict_v16a_best_checkpoint" "${PREDICT_COMMAND[@]}"
fi
if [[ "${SKIP_EVAL}" != "1" ]]; then
