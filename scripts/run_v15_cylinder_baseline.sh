...
EXPERIMENT_NAME="trainval_v15_cylinder_1024"
TRAIN_START_IDX=0
...
usage() {
  cat <<'EOF'
Run V15 Cylinder-style supervised LiDAR baseline.

V15 replaces the Cartesian ResUNet baseline with a Cylinder3D-style spconv U-Net:
  - raw nuScenes LiDAR + lidarseg labels
  - xyz + intensity input
  - cylindrical voxel grid [radius, azimuth, z]
  - weighted CE + optional Lovasz/Dice
  - in-training eval and best checkpoint
...
LOG_FILE="${LOG_DIR}/v15_${EXPERIMENT_NAME}_${TIMESTAMP}.log"
LATEST_LOG="${LOG_DIR}/v15_${EXPERIMENT_NAME}_latest.log"
...
TRAIN_CORE=(
  ...
  --point_cloud_range 0.0 -3.141592653589793 -5.0 60.0 3.141592653589793 3.0
...
{
  echo "[INFO] V15 Cylinder baseline started at $(date -Is)"
  echo "[INFO] project_root=${PROJECT_ROOT}"
  echo "[INFO] experiment_dir=${EXPERIMENT_DIR}"
  echo "[INFO] backbone=cylinder_spconv_unet"
  echo "[INFO] data_source=raw_lidarseg"
...
if [[ "${SKIP_TRAIN}" != "1" ]]; then
  run_step "train_v15_cylinder_spconv_unet" "${TRAIN_COMMAND[@]}"
fi
if [[ "${SKIP_PREDICT}" != "1" ]]; then
  ...
  run_step "predict_v15_best_checkpoint" "${PREDICT_COMMAND[@]}"
fi
if [[ "${SKIP_EVAL}" != "1" ]]; then
  run_step "eval_v15_best_checkpoint" "${EVAL_COMMAND[@]}"
fi

{
  echo "[INFO] V15 Cylinder baseline finished at $(date -Is)"
