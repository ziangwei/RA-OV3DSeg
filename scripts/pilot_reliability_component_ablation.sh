...
PILOT_SUBSET_ROOT="${PILOT_SUBSET_ROOT:-${PROJECT_ROOT}/outputs/pointcept/reliability_component_subset_128}"
PILOT_OUT_ROOT="${PILOT_OUT_ROOT:-${PROJECT_ROOT}/outputs/pointcept/reliability_component_pilot}"
PILOT_LOG_DIR="${PILOT_LOG_DIR:-${PROJECT_ROOT}/outputs/logs/reliability_component_pilot}"
...
safe_rm_path() {
  ...
  case "${target}" in
    "${PROJECT_ROOT}/outputs/"*)
      ...
      ;;
    *)
      echo "[component][ERROR] refusing to remove path outside outputs/: ${target}" >&2
      exit 2
      ;;
  esac
}
...
SUMMARY_TSV="${PILOT_OUT_ROOT}/pilot_component_summary.tsv"
printf "component_mode\tstatus\tgate_passed\tval_miou\tdistill_valid_ratio\tdistill_mean_weight\tlog\n" > "${SUMMARY_TSV}"
...
echo "[component] building and preflighting cached subset; log: ${preflight_log}"
set +e
{
...
if missing_names:
    raise SystemExit(
        "[component][ERROR] subset contains non-cache-resolvable names: "
        + ", ".join(missing_names[:8])
    )
if missing_cache:
    raise SystemExit(
        "[component][ERROR] subset references samples without cache: "
        + ", ".join(sorted(set(missing_cache))[:8])
    )
if duplicate_names:
    raise SystemExit("[component][ERROR] subset has duplicate cache sample ids: " + "; ".join(duplicate_names[:8]))
if bad_lidar:
    raise SystemExit("[component][ERROR] cannot read pilot lidar/cache inputs: " + "; ".join(bad_lidar[:4]))
if point_mismatch:
    raise SystemExit(
        "[component][ERROR] point/cache alignment mismatch before training: "
        + "; ".join(point_mismatch[:4])
    )
if not train_infos or not val_infos:
    raise SystemExit("[component][ERROR] subset train/val split is empty")

print(
    f"[component] preflight ok: train={len(train_infos)} val={len(val_infos)} "
...
if [ "${preflight_exit}" -eq 0 ]; then
  ...
  echo "[component][ERROR] preflight failed: ${preflight_reason}"
  echo "[component][ERROR] full log: ${preflight_log}"
fi

if [ "${run_status}" = "success" ]; then
for component_mode in ${COMPONENT_MODES}; do
  tag="${COMPONENT_THRESHOLD/./p}"
  run_name="pilot_reliability_component_${component_mode}_t${tag}"
  ...
  echo "[component] mode=${component_mode} threshold=${COMPONENT_THRESHOLD} running; full log: ${console_log}"
  set +e
...
print("\t".join([component_mode, status, gate_passed, val_miou, valid, weight, str(log_path)]))
PY
)
  printf "%s\n" "${parsed}" >> "${SUMMARY_TSV}"

  IFS=$'\t' read -r parsed_component parsed_status parsed_gate parsed_miou parsed_valid parsed_weight parsed_log <<< "${parsed}"
  echo "[component] mode=${parsed_component} status=${parsed_status} val_miou=${parsed_miou} distill_valid_ratio=${parsed_valid}"

  if [ "${exit_code}" -ne 0 ] || [ "${parsed_status}" != "success" ] || [ "${parsed_gate}" != "yes" ]; then
    run_status="failed"
    failed_component="${component_mode}"
    failed_log="${console_log}"
    failure_reason="$(extract_failure_reason "${console_log}")"
    echo "[component][ERROR] mode=${component_mode} failed: ${failure_reason}"
    echo "[component][ERROR] full log: ${console_log}"
    if [ "${PILOT_VERBOSE_FAILURE}" = "1" ]; then
      echo "[component][ERROR] last 60 log lines:"
...
python - "${SUMMARY_TSV}" "${run_status}" "${failed_component}" "${failed_log}" "${COMPONENT_THRESHOLD}" <<'PY'
from __future__ import annotations
...
max_drop_row = max(drop_rows, key=lambda item: item[1]) if drop_rows else ("-", 0.0, 0.0)
...
print("[component] compact summary:")
for row in rows:
    print(
        f"[component] mode={row['component_mode']} status={row['status']} "
...
conclusion = RunConclusion(
    stage="stage-reliability",
    experiment="pilot_reliability_component_ablation",
    status="success" if run_status == "success" else "failed",
    gate="128-cache pilot component ablation shows at least one component matters; not the final Stage 4 gate",
    gate_passed=gate_passed,
    primary_metric_name="max_component_drop_vs_full",
    ...
    artifacts=artifacts,
    next_step=(
        "scale to a 512-cache reliability run if the component signal is stable"
        if gate_passed
        else "inspect component table and decide whether to stop Stage 4 at pilot evidence"
        if run_status == "success"
        else "fix component pilot failure before launching more runs"
    ),
    notes=notes,
)
