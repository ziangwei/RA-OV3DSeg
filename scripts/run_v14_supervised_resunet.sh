...
EXPERIMENT_NAME="trainval_v14_supervised_resunet_128"
TRAIN_START_IDX=0
...
usage() {
  cat <<'EOF'
Run V14 supervised spconv_resunet recipe.

V14 focuses on the 3D closed-set upper bound:
  - class-balanced CE from computed class frequencies
  - optional Lovasz/Dice losses
  - basic LiDAR augmentation
  - in-training eval and best checkpoint by eval mIoU
...
mkdir -p "${TRAINING_DIR}" "${PREDICTION_DIR}" "${EVALUATION_DIR}" "${RESULTS_DIR}" "${LOG_DIR}"

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="${LOG_DIR}/v14_${EXPERIMENT_NAME}_${TIMESTAMP}.log"
LATEST_LOG="${LOG_DIR}/v14_${EXPERIMENT_NAME}_latest.log"
...
CHECKPOINT="${TRAINING_DIR}/spconv_resunet_best.pt"
PREDICT_COMMAND=(
...
{
  echo "[INFO] V14 supervised ResUNet started at $(date -Is)"
  ...
  echo "[INFO] lovasz_weight=${LOVASZ_WEIGHT} dice_weight=${DICE_WEIGHT} augment=${AUGMENT}"
...
if [[ "${SKIP_TRAIN}" != "1" ]]; then
  run_step "train_v14_spconv_resunet" "${TRAIN_COMMAND[@]}"
fi
if [[ "${SKIP_PREDICT}" != "1" ]]; then
  ...
  run_step "predict_v14_best_checkpoint" "${PREDICT_COMMAND[@]}"
fi
if [[ "${SKIP_EVAL}" != "1" ]]; then
  run_step "eval_v14_best_checkpoint" "${EVAL_COMMAND[@]}"
fi

{
  echo "[INFO] V14 supervised ResUNet finished at $(date -Is)"
